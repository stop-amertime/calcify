...
pub fn parse_property_body<'i, 't>(name: &str, input: &mut Parser<'i, 't>) -> Result<PropertyDef> {
    ...
    while !input.is_exhausted() {
        let descriptor = input
            .expect_ident_cloned()
            .map_err(|e| crate::CalciteError::Parse(format!("expected descriptor name: {e:?}")))?;

        input
            .expect_colon()
            .map_err(|e| crate::CalciteError::Parse(format!("expected ':': {e:?}")))?;

        match &*descriptor {
            "syntax" => {
                let s = input.expect_string_cloned().map_err(|e| {
                    crate::CalciteError::Parse(format!("expected syntax string: {e:?}"))
                })?;
                syntax = parse_syntax(&s)?;
            }
            "inherits" => {
                let ident = input.expect_ident_cloned().map_err(|e| {
                    crate::CalciteError::Parse(format!("expected 'true' or 'false': {e:?}"))
                })?;
...
fn parse_initial_value<'i, 't>(input: &mut Parser<'i, 't>) -> Result<CssValue> {
    ...
    match input.next().cloned() {
        ...
        Err(e) => Err(crate::CalciteError::Parse(format!(
