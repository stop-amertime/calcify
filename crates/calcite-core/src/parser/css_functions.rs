...
type CssParseError<'i> = cssparser::ParseError<'i, crate::CalciteError>;

/// Helper: run parse_nested_block with CalciteError as the custom error.
...
/// Convert cssparser::ParseError<CalciteError> into CalciteError.
fn css_err(e: CssParseError<'_>) -> crate::CalciteError {
    match e.kind {
        cssparser::ParseErrorKind::Custom(e) => e,
        other => crate::CalciteError::Parse(format!("{other:?}")),
    }
}

/// Convert a BasicParseError to CalciteError (for use inside closures).
fn basic_err(e: cssparser::BasicParseError<'_>) -> crate::CalciteError {
    crate::CalciteError::Parse(format!("{e:?}"))
}

/// Wrap a CalciteError into a CssParseError for use inside parse_nested_block closures.
fn wrap_err<'i>(e: crate::CalciteError, loc: cssparser::SourceLocation) -> CssParseError<'i> {
...
fn parse_atom<'i, 't>(input: &mut Parser<'i, 't>) -> Result<Expr> {
    ...
    let token = input
        .next()
        .map_err(|e| crate::CalciteError::Parse(format!("unexpected end of input: {e:?}")))?
        .clone();
    ...
    match token {
        ...
        other => {
            input.reset(&state);
            Err(crate::CalciteError::Parse(format!(
...
fn parse_function_call<'i, 't>(input: &mut Parser<'i, 't>, name: &str) -> Result<Expr> {
    nested_block(input, |inner| {
        let result = match name {
            ...
            _ => Err(crate::CalciteError::Parse(format!(
...
fn parse_var<'i, 't>(input: &mut Parser<'i, 't>) -> Result<Expr> {
    ...
    if !name.starts_with("--") {
        return Err(crate::CalciteError::Parse(format!(
        ...
        match parse_style_condition(input) {
            ...
            Err(_) => {
                ...
                if branches.is_empty() {
                    return Err(crate::CalciteError::Parse(
