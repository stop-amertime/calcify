...
fn main() {
    ...
    let parsed = calcite_core::parser::parse_css(&css).expect("should parse");
    ...
    for a in &parsed.assignments {
        match &a.value {
            calcite_core::types::Expr::StyleCondition { branches, .. } => {
                style_cond_count += 1;
                if branches.len() > 100 {
                    println!(
            ...
            calcite_core::types::Expr::StringLiteral(_) => literal_count += 1,
            calcite_core::types::Expr::Var { .. } => var_count += 1,
